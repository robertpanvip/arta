import {PathSegment, PointLike} from './interface';
import {
    getCubicArcLength,
...
export class Segment {
    ...
    _readDataAttribute() {
        this.dataArray = Segment.parsePathData(this._data);
        this.pathLength = Segment.getPathLength(this.dataArray);
    }

    static getBBox(dataArray: PathSegment[]) {
            ...
            if (data.command === 'A') {
                ...
                if (dTheta < 0) {
                    ...
                    for (let t = start - inc; t > end; t -= inc) {
                        const point = Segment.getPointOnEllipticalArc(
                            data.points[0],
                            data.points[1],
                            data.points[2],
                            data.points[3],
                ...
                for (let t = 0.0; t <= 1; t += 0.01) {
                    const point = Segment.getPointOnCubicBezier(
                        t,
                        data.start.x,
                        data.start.y,
                        data.points[0],
    ...
    getSelfRect() {
        return Segment.getBBox(this.dataArray)
    }
    ...
    getPointAtLength(length: number) {
        return Segment.getPointAtLengthOfDataArray(length, this.dataArray);
    }
    ...
    static getPointAtLengthOfDataArray(length: number, dataArray: PathSegment[]) {
        ...
        switch (cp.command) {
            case 'L':
                return Segment.getPointOnLine(length, cp.start.x, cp.start.y, p[0], p[1]);
            case 'C':
                return Segment.getPointOnCubicBezier(
                    t2length(length, Segment.getPathLength(dataArray), (i) => {
            ...
            case 'Q':
                return Segment.getPointOnQuadraticBezier(
                    t2length(length, Segment.getPathLength(dataArray), (i) => {
            ...
            case 'A':
                ...
                return Segment.getPointOnEllipticalArc(cx, cy, rx, ry, theta, psi);
        }
    ...
    static parsePathData(data: string): PathSegment[] {
        ...
        for (let n = 1; n < arr.length; n++) {
            ...
            while (p.length > 0) {
                ...
                let cmd: PathSegment['command'] | null = null;
                let points: number[] = [];
                const startX = cpx,
                    startY = cpy;
                ...
                const command = cmd! || c as PathSegment["command"]
                ...
                ca.push({
                    ...
                    pathLength: this.calcLength(startX, startY, command, points),
                });
            }
            ...
            if (c === 'z' || c === 'Z') {
                ca.push({
                    command: 'z',
                    points: [],
                    start: undefined as unknown as PointLike,
                    pathLength: 0,
                });
            }
        }
    ...
    static calcLength(x: number, y: number, cmd: PathSegment["command"], points: number[]) {
        let len, p1, p2, t;
        const path = Segment;
