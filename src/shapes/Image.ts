...
class Image extends Rect {
    ...
    set xlinkHref(xlinkHref: string) {
        ...
        const loop = () => {
            timer = setTimeout(() => {
                vm.get(this)!.angle = vm.get(this)!.angle + 60;
            }, 50)
        }
        loop();
        const onLoad = () => {
            const w = this.style.width || 0;
            const h = this.style.height || 0;
            ...
            if (!w) {
                if (h) {
                    this.style.width = img.width * h / img.height
                } else {
            ...
            if (!h) {
                if (w) {
                    this.style.height = img.height * w / img.width
                } else {
    ...
    render(ctx: Context) {
        ...
        if (scope.status === Status.loaded) {
            if (scope.resource) {
                ctx.drawImage(vm.get(this)!.resource!, 0, 0, w, h!)
            }
        }
